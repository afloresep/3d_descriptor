...
def _compute_MC2(mol):
    ...
    Where
        ...
        DV = number of divalent nodes 
            (like bridging atoms in the molecular graph),
        X-C=O) = number of carbonyl + heteroatom “carboxyl-like” groups 
            (amides, esters, carboxylic acids, etc.).
        It is more nuanced than a raw size measure because it penalizes certain 
        structural motifs
...
def _compute_fraction_sp3(mol: Chem.Mol) -> float:
    ...
    """
    sp3_count = sum(1 for atom in mol.GetAtoms() if
...
def _compute_fraction_carbon(mol: Chem.Mol) -> float:
    ...
    """
    carbon_count = sum(1 for atom in mol.GetAtoms() if
                       atom.GetAtomicNum() == 6)
    hac = _get_heavy_atom_count(mol)
